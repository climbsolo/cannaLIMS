...
if (isset($_SESSION["role"])) {    
    if ($_SESSION["role"] == 'Admin') {        
        ?>        
        <select id="customerportaldropdown" onchange="showportalinfo()"></select>      
        <?php        
    }  
}
?>
  ...
  <ul class="nav nav-tabs" role="tablist">
    ...
    <li role="presentation"><a href="#profile" aria-controls="profile" role="tab" data-toggle="tab" onclick="getdistributionlist(),$('#searchoptionsdiv').hide();getinvoices();">Profile</a></li>
  </ul>
    ...
    <div role="tabpanel" class="tab-pane" id="profile">
        ...
        <div class="row">
            <div class="col-lg-6">
                ...
                <h3 style="text-align:left;">Manage Distribution List <button id="btnadddistributionlistemail" class="btn btn-primary" style="font-size:80%;margin-left:1em;">Add New</button></h3>
                <div id="managedistributionlistdiv" style="display:none;">                
                    <fieldset class="fieldset" style="padding:1em;">
                        ...
                        <input type="checkbox" id="dlaccountant" style="vertical-align:middle;">
                        <label for="dlaccountant"> Receive Invoice Notifications</label></input>                           
                        </div>
            ...
            <div class="col-lg-6">     
                <div>
                    <h3>Invoices</h3>
                    <input id="invoices_currentcount" style="display:none;"></input>
                    <div><a href="javascript:void(0)" onclick="showunbilledsamples()">Pending Samples (Not Yet Invoiced)</a>
                    <br /><br />
                    <div id="unbilledsamples_div"></div>
                    <div id="invoices"></div>                        
                    </div>
                </div>
            </div>        
...
function applydistributionlistitem() {
    ...
    var receive_invoice_notifications = $("#dlaccountant").prop("checked"); 
    
    
    $.post("customeradddistributionlistitem.php", {email: email, receive_invoice_notifications: receive_invoice_notifications},
    function(data) {
        $("#distributionlist").html(data); 
        hideadddistributionlistitem();
        getdistributionlist();
...
var currentcount = $("#invoices_currentcount").val();
    ...
    $.post("customergetinvoices.php", {currentcount: currentcount}, 
    function(data) {  
        ...
        $("#invoices_currentcount").val(d[0]);
        
        $("#invoices").html($("#invoices").html() + d[1]);
        
    });   
}

function customerportaldropdowninit() {
    $(document).ready(function() {               
        $.getJSON("adminfunctions.php?calltype=getcustomerportaldropdown", 
        function(data) {            
            $.each(data, function(index, item) {
                $("#customerportaldropdown").append(function() {                          
                return '<option value=' + item.val + '>' + item.text + '</option>';
                });
            });
  $("#customerportaldropdown").val("<?php if (isset($_SESSION["cluid"])) {echo $_SESSION["cluid"];} ?>");            
        });     
    });
}
...
function showportalinfo() {
    ...
    var license_number = $("#customerportaldropdown").val();
    
    $.get("adminfunctions.php?calltype=adminportallogin&license_number=" + license_number, 
    function() {
        location.reload();
    });    
}

function showunbilledsamples() {
    
     waitingDialog.show();
    
    $.get("customergetpendingsamples.php", 
    function(data) {
        waitingDialog.hide();
        $("#unbilledsamples_div").html(data);        
    });    
}

function showsampledetails(id, qbid) {
    
  $.get("customergetinvoicedetails.php?qbid="+qbid, 
    function(data) {
        $("#" + id).html(data);
    });    
}
