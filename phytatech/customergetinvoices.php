...
$sql = "select distinct quickbooks_invoice_id from tblsamples where (sent_to_quickbooks = 1) and (active is null or active = '' or active <> 'false') and (license_number = :cluid) and (quickbooks_invoice_id is not null) limit $currentcount, $nextcount";
...
if ($stmt->execute(array(':cluid'=>$cluid))) {
    while ($row = $stmt->fetch()) { 
        ...
        $sql1 = "select min(ndate_report_approval_workflow) as min, max(ndate_report_approval_workflow) as max from tblsamples where quickbooks_invoice_id='$quickbooks_invoice_id'";
