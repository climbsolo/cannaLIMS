...
if ($stmt->execute()) {
    while ($row = $stmt->fetch()) {
        ...
        $sql2 = "update tblreports set notified = 'true' where licensenumber = :licensenumber";
        $stmt2 = $dbconn->prepare($sql2);
        $stmt2->execute(array(':licensenumber'=>$licensenumber));
...
if ($stmt4->execute()) {
    while ($row4 = $stmt4->fetch()) {
        ...
        $sql3 = "select sample_id from tblsamples where (active is null or active = '' or active <> 'false') and quickbooks_insert_date=curdate() and license_number = :licensenumber group by license_number";
